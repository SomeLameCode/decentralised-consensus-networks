"""
sim_v19c_D5_longrun.py -- Phase 3 Series A2: D=5 run (Run 57)

Validation: should reproduce Run 54 (sim_v18_seed42_ultralong.py) exactly.
Same seed=42, same D=5, same T=20000, same rng draw order -> identical results.

Expected: T1 conv at t~1626, T3 conv at t~11299, T2 conv at t~13574.
...
Outputs (workspace/simulation/phase-3/):
  sim_v19c_D5_longrun_plots.png
  D5-longrun-summary.csv

Run from project root:
  python workspace/simulation/phase-3/sim_v19c_D5_longrun.py
"""
...
# -- Helper functions (unchanged from sim_v14-v19b) ---------------------------
...
print("=" * 72)
print(f"sim_v19c_D5_longrun.py -- D=5 (Run 57) | seed={SEED} | T={T}")
print(f"N={N}  M={M}  D={D}  ALPHA={ALPHA}  ETA={ETA}  KAPPA={KAPPA}")
print(f"Milestone every {MILESTONE_STEP} steps")
print(f"Validation: should reproduce Run 54 (t_conv: T1~1626, T3~11299, T2~13574)")
print("=" * 72)
...
print(f"\nValidation vs Run 54 expected (T1:1626, T3:11299, T2:13574):")
for k in range(M):
    conv_t = per_topic_conv[k]
    print(f"  {TOPIC_LABELS[k]}: t_conv={conv_t}")
...
csv_path = os.path.join(OUTPUT_DIR, "D5-longrun-summary.csv")
...
fig.suptitle(
    f"sim_v19c_D5_longrun.py -- D=5 (Run 57) | seed={SEED} | T={T}\n"
...
plt.tight_layout()
plot_path = os.path.join(OUTPUT_DIR, "sim_v19c_D5_longrun_plots.png")
plt.savefig(plot_path, dpi=150, bbox_inches="tight")
print(f"Plots saved -> {plot_path}")
print(f"\n-- sim_v19c_D5_longrun.py complete --")
