"""
sim_v19b_D2_longrun.py -- Phase 3 Series A2: D=2 extended run T=20000

Context: Run 56 (D=2, T=2000) showed Topic 1 plateauing at cosim=0.884
without reaching convergence threshold, while Topics 2 and 3 stayed near
zero (0.04-0.08) throughout. This run extends to T=20000 to determine:
  (a) Does Topic 1 cascade to full convergence, or is 0.88 a ceiling?
  (b) Do Topics 2 and 3 eventually cascade (same pattern as D=5 slow class)?

Fixed: N=30, M=3, D=2, T=20000, ALPHA=0.3, ETA=0.15, LAMBDA=0.1, KAPPA=0.0
       Gaussian init, WS k=4 p=0.1, seed=42

Label: D=2 long run (T=20000) -- extends Run 56

Outputs (workspace/simulation/phase-3/):
  sim_v19b_D2_longrun_plots.png  -- single panel, 3 topic lines
  D2-longrun-summary.csv

Run from project root:
  python workspace/simulation/phase-3/sim_v19b_D2_longrun.py
"""
...
# -- Helper functions (unchanged from sim_v14-v19) ----------------------------
...
print("=" * 72)
print(f"sim_v19b_D2_longrun.py -- D=2 extended | seed={SEED} | T={T}")
...
csv_path = os.path.join(OUTPUT_DIR, "D2-longrun-summary.csv")
...
fig.suptitle(
    f"sim_v19b_D2_longrun.py -- D=2 extended | seed={SEED} | T={T}\n"
...
plt.tight_layout()
plot_path = os.path.join(OUTPUT_DIR, "sim_v19b_D2_longrun_plots.png")
plt.savefig(plot_path, dpi=150, bbox_inches="tight")
print(f"Plots saved -> {plot_path}")
print(f"\n-- sim_v19b_D2_longrun.py complete --")
