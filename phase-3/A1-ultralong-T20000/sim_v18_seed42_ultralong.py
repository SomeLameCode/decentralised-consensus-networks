"""
sim_v18_seed42_ultralong.py -- Phase 3 Series A1 extended: seed 42, T=20000

Context: Run 52 (T=5000, seed=42) showed Topic 1 converging by t~2000,
while Topics 2 and 3 remained flat/oscillating (cosim 0.12-0.35) with
no clear convergence trend through T=5000. This run extends to T=20000
to determine whether Topics 2 and 3 represent a genuinely frustrated
regime or ultra-slow convergence on a very long timescale.

Fixed: N=30, M=3, D=5, T=20000, ALPHA=0.3, ETA=0.15, LAMBDA=0.1, KAPPA=0.0
       Gaussian init, WS k=4 p=0.1, seed=42

Run number: 54

Outputs (workspace/simulation/phase-3/):
  sim_v18_seed42_ultralong_plots.png  -- single panel, 3 topic lines
  seed42-ultralong-summary.csv

Run from project root:
  python workspace/simulation/phase-3/sim_v18_seed42_ultralong.py
"""
...
SNAPSHOT_TIMES = [500, 1000, 2000, 5000, 10000, 15000, 20000]
MILESTONE_STEP = 2000

OUTPUT_DIR = os.path.dirname(os.path.abspath(__file__))

# -- Helper functions (unchanged from sim_v14-v17) ----------------------------
...
print("=" * 72)
print(f"sim_v18_seed42_ultralong.py -- Run {RUN_NUM} | seed={SEED} | T={T}")
print(f"N={N}  M={M}  D={D}  ALPHA={ALPHA}  ETA={ETA}  KAPPA={KAPPA}")
print(f"Milestone snapshots every {MILESTONE_STEP} steps")
print("=" * 72)
...
t_conv_all = int(conv_steps[0]) if len(conv_steps) > 0 else None
...
print(f"\n{'=' * 72}")
print(f"SUMMARY -- Run {RUN_NUM}  seed={SEED}  T={T}  ETA={ETA}  D={D}")
print(f"\nFinal cosim at T={T}:")
for k in range(M):
...
print(f"\nCosim snapshots:")
header = "  ".join(f"t={snap_t:>6}" for snap_t in SNAPSHOT_TIMES)
print(f"  {'':12}  {header}")
for k in range(M):
    vals = "  ".join(f"{snapshots[snap_t][k]:>8.3f}" for snap_t in SNAPSHOT_TIMES)
    print(f"  {TOPIC_LABELS[k]:<12}  {vals}")

print(f"\never-SME: {ever_sme}  (min={min(ever_sme)}/{N})")
print(f"top_holder: {[round(x, 3) for x in top_holder]}")
print(f"div={diversity_regime}  conv_any={converged_flag}  t_conv_all={t_conv_all}")

# -- Write CSV ----------------------------------------------------------------

csv_path = os.path.join(OUTPUT_DIR, "seed42-ultralong-summary.csv")
with open(csv_path, "w", newline="") as f:
    writer = csv.writer(f)
    header_row = ["run_num", "seed"]
    for snap_t in SNAPSHOT_TIMES:
        for k in range(M):
            header_row.append(f"T{k+1}_t{snap_t}")
    header_row += ["ever_sme_t1", "ever_sme_t2", "ever_sme_t3",
                   "t_conv_t1", "t_conv_t2", "t_conv_t3",
                   "t_conv_all", "diversity_regime", "converged"]
    writer.writerow(header_row)

    data_row = [RUN_NUM, SEED]
    for snap_t in SNAPSHOT_TIMES:
        for k in range(M):
            data_row.append(f"{snapshots[snap_t][k]:.4f}")
    data_row += [
        ...
        t_conv_all if t_conv_all is not None else "NA",
        int(diversity_regime),
        int(converged_flag),
    ]
...
fig.suptitle(
    f"sim_v18_seed42_ultralong.py -- Run {RUN_NUM} | seed={SEED} | T={T}\n"
...
ax.axhline(CONV_THRESHOLD, color="black", linestyle="--",
           linewidth=0.8, alpha=0.8, label=f"conv threshold ({CONV_THRESHOLD})")
ax.axhline(0.5, color="grey", linestyle=":", linewidth=0.7, alpha=0.7,
           label="0.5 diversity boundary")

for snap_t in SNAPSHOT_TIMES[:-1]:
...
plt.tight_layout()
plot_path = os.path.join(OUTPUT_DIR, "sim_v18_seed42_ultralong_plots.png")
plt.savefig(plot_path, dpi=150, bbox_inches="tight")
print(f"Plots saved -> {plot_path}")
print(f"\n-- sim_v18_seed42_ultralong.py complete --")
