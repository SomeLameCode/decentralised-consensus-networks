"""
sim_v1.py — Decentralised Consensus Learning Network (Phase 1 PoC)
...
Parameters locked per model-spec.md:
  N=30, M=3, T=200, alpha=0.3, eta=0.05, lambda=0.1, epsilon=0.3
...
ETA = 0.05        # innovation rate
LAMBDA = 0.1      # competence plasticity
EPSILON = 0.3     # bounded confidence threshold (D-001)
...
fig.suptitle(
    "sim_v1.py — Decentralised Consensus Learning Network\n"
    "N=30, M=3, T=200 | Watts-Strogatz(k=4,p=0.1) | bounded confidence ε=0.3",
    fontsize=10
)
...
plt.savefig(os.path.join(_HERE, "sim_v1_plots.png"), dpi=150, bbox_inches="tight")
print(f"Plots saved -> {os.path.join(_HERE, 'sim_v1_plots.png')}")

# ── CSV export (Session 52 backfill — same values as the console printout above) ──

import csv

csv_path = os.path.join(_HERE, "sim_v1_summary.csv")
