"""
sim_v2.py — Decentralised Consensus Learning Network (Phase 1 PoC, Run 2)
...
Parameters locked per model-spec.md (changes from Run 1 in brackets):
  N=30, M=3, T=200, alpha=0.3, eta=0.15 [was 0.05], lambda=0.1, epsilon=0.15 [was 0.3]
...
ETA = 0.15        # innovation rate (was 0.05 in Run 1)
LAMBDA = 0.1      # competence plasticity
EPSILON = 0.15    # bounded confidence threshold (was 0.3 in Run 1)
...
for t in range(T):
    ...
    for k in range(M):
        ...
        # Belief update: decay + social consensus + small innovation
        social = W_k @ B[:, k]
...
fig.suptitle(
    "sim_v2.py — Decentralised Consensus Learning Network\n"
    "N=30, M=3, T=200 | Watts-Strogatz(k=4,p=0.1) | ε=0.15  η=0.15",
    fontsize=10
)
...
plt.savefig(os.path.join(_HERE, "sim_v2_plots.png"), dpi=150, bbox_inches="tight")
print(f"Plots saved -> {os.path.join(_HERE, 'sim_v2_plots.png')}")

# ── CSV export (Session 52 backfill — same values as the console printout above) ──

import csv

csv_path = os.path.join(_HERE, "sim_v2_summary.csv")
