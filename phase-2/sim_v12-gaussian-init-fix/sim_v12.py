"""
sim_v12.py — Decentralised Consensus Learning Network (Phase 2, Run 12)

Vector beliefs with corrected initialisation.
Fixes the positive-orthant bias in sim_v11.py: beliefs now initialised
from N(0,1) and normalised, giving uniform spherical distribution
with expected mean pairwise cosim ≈ 0 at t=0.

Direct comparison: Run 11 (uniform[0,1] init, cosim ~0.75 at t=0)
                   Run 12 (N(0,1) init, cosim ~0.0 at t=0)

Parameters: identical to Run 11.
"""
...
print("\nComparison:")
print("  Run 11 (uniform[0,1] init): cosim ~0.75 at t=0  [positive-orthant bias]")
vec_str = " | ".join(
    f"{TOPIC_LABELS[k]}: {mean_cosim_hist[0, k]:.3f} -> {mean_cosim_hist[-1, k]:.3f}"
    for k in range(M)
)
print(f"  Run 12 (Gaussian init):     cosim  {vec_str}")
...
fig.suptitle(
    "sim_v12.py — Vector Beliefs (D=5, Gaussian init)  |  Run 12\n"
    "N=30, M=3, T=200, D=5 | Watts-Strogatz(k=4,p=0.1) | "
    "KAPPA=0.5 (cosine BC filter)  Gaussian init",
    fontsize=10
)
...
plt.savefig(os.path.join(_HERE, "sim_v12_plots.png"),
            dpi=150, bbox_inches="tight")
print(f"\nPlots saved -> {os.path.join(_HERE, 'sim_v12_plots.png')}")

# -- CSV export (Session 52 backfill -- same values as the console printout above) --

import csv

csv_path = os.path.join(_HERE, "sim_v12_summary.csv")
