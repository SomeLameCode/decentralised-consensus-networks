"""
sim_v11.py — Decentralised Consensus Learning Network (Phase 2, Run 11)

Vector beliefs: scalar beliefs b_i(k,t) in [0,1] upgraded to unit vectors
b_i(k,t) in R^D, D=5. First vector-belief run. Direct comparison baseline: Run 1.

Changes from scalar baseline (sim_v1.py):
  - B shape: (N, M, D), normalised to unit sphere after every update
  - Bounded confidence: cosine similarity >= KAPPA (not |diff| <= epsilon)
  - Innovation noise: Gaussian in R^D (not scalar uniform)
  - Peer-consistency delta: dot product, positive (higher alignment = better)
  - Belief diversity: mean pairwise cosine similarity (not Shannon entropy)
  - CONV_THRESHOLD used only to detect t_conv — never in the simulation loop
  - Isolated-node guard carried from sim_v10.py (delta=nan -> scaled=0.0)

Parameters: identical to Run 1 where applicable for direct comparison.
  N=30, M=3, T=200, alpha=0.3, eta=0.15, lambda=0.1, seed=42
  Graph: Watts-Strogatz(k=4, p=0.1)
  New: D=5 (belief vector dimension), KAPPA=0.5 (cosine BC filter)
"""
...
KAPPA = 0.5         # bounded confidence filter: per-step interaction gate (cosim >= KAPPA)
...
B_raw = rng.uniform(0.0, 1.0, (N, M, D))
norms_init = np.linalg.norm(B_raw, axis=2, keepdims=True)
B = B_raw / norms_init                           # unit vectors, shape (N, M, D)
...
def apply_bounded_confidence_cosine(W, B_k, kappa):
    """
    Zero out trust to neighbours whose cosine similarity < kappa (KAPPA=0.5).
    B_k: (N, D) unit vectors for one topic.
    cosim_ij = dot(b_i, b_j) since both are unit vectors.
...
print("\nComparison:")
print("  Run 1  (scalar): entropy ~2.200 -> 0.000  [full consensus by T=200]")
vec_str = " | ".join(
    f"{TOPIC_LABELS[k]}: {mean_cosim_hist[0, k]:.3f} -> {mean_cosim_hist[-1, k]:.3f}"
    for k in range(M)
)
print(f"  Run 11 (vector): cosim  {vec_str}")
...
fig.suptitle(
    "sim_v11.py — Vector Beliefs (D=5)  |  Run 11\n"
    "N=30, M=3, T=200, D=5 | Watts-Strogatz(k=4,p=0.1) | "
    "KAPPA=0.5 (cosine BC filter)  eta=0.15",
    fontsize=10
)

# Row 0: belief vector heatmaps at t=0, 100, 200 (Topic 1)
# Each panel shows B_hist[snap, :, 0, :] — nodes x D for Topic 1
...
plt.savefig(os.path.join(_HERE, "sim_v11_plots.png"),
            dpi=150, bbox_inches="tight")
print(f"\nPlots saved -> {os.path.join(_HERE, 'sim_v11_plots.png')}")

# -- CSV export (Session 52 backfill -- same values as the console printout above) --

import csv

csv_path = os.path.join(_HERE, "sim_v11_summary.csv")
