"""
sim_v14.py — Decentralised Consensus Learning Network (Phase 2, Run 14)

Vector beliefs, Gaussian init, KAPPA=0.0, T=1000.

Run 13 showed that KAPPA=0.0 restores social learning (Topic 1 cosim reached
0.630 by T=200) but T=200 is too short for full convergence. Topics 2 and 3
were still at 0.118 and 0.221 — clearly mid-trajectory. The transient oligarchy
observed (T1 top holders: 92%, 81%) is expected during mid-convergence.

Run 14 extends to T=1000 to let the network fully converge and observe
post-convergence rotation. The scalar finding was that rotation strengthens
after convergence; this run tests whether that holds for vector beliefs.

Direct comparison:
  Run 11 (uniform init, KAPPA=0.5, T=200): conv t=14, 57-73% post-conv rotation
  Run 12 (Gaussian init, KAPPA=0.5, T=200): frozen, no convergence
  Run 13 (Gaussian init, KAPPA=0.0, T=200): mid-convergence, transient oligarchy
  Run 14 (Gaussian init, KAPPA=0.0, T=1000): full convergence expected

Parameters: identical to Run 13 except T=200 -> T=1000.
"""
...
def apply_bounded_confidence_cosine(W, B_k, kappa):
    ...
    the filter passes all neighbours on an undirected graph.
...
for k in range(M):
    ...
    print(f"  {TOPIC_LABELS[k]}: {cosim_t0:.3f}  "
          f"(Run 13 reference: ~0.00 | KAPPA={KAPPA} — filter {'OFF' if KAPPA == 0.0 else 'ON'})")
...
print("\nMean pairwise cosim (t=0 -> t=1000):")
for k in range(M):
    print(f"  {TOPIC_LABELS[k]}: {mean_cosim_hist[0, k]:.3f} -> {mean_cosim_hist[-1, k]:.3f}")

print("\nComparison:")
print("  Run 11 (uniform init, KAPPA=0.5, T=200):  cosim ~0.75 -> 0.997  conv t=14")
print("  Run 12 (Gaussian init, KAPPA=0.5, T=200): cosim ~0.00 -> 0.035  NO convergence")
print("  Run 13 (Gaussian init, KAPPA=0.0, T=200): cosim ~0.00 -> 0.630/0.118/0.221  mid-conv")
vec_str = " | ".join(
    f"{TOPIC_LABELS[k]}: {mean_cosim_hist[0, k]:.3f} -> {mean_cosim_hist[-1, k]:.3f}"
    for k in range(M)
)
print(f"  Run 14 (Gaussian init, KAPPA=0.0, T=1000): cosim  {vec_str}")
...
fig.suptitle(
    "sim_v14.py — Vector Beliefs (D=5, Gaussian init, KAPPA=0.0)  |  Run 14\n"
    "N=30, M=3, T=1000, D=5 | Watts-Strogatz(k=4,p=0.1) | "
    "KAPPA=0.0 (BC filter OFF)  Gaussian init",
    fontsize=10
)

# Row 0: belief vector heatmaps at t=0, 500, 1000 (Topic 1)
for col, snap in enumerate([0, 500, 1000]):
...
plt.savefig(os.path.join(_HERE, "sim_v14_plots.png"),
            dpi=150, bbox_inches="tight")
print(f"\nPlots saved -> {os.path.join(_HERE, 'sim_v14_plots.png')}")

# -- CSV export (Session 52 backfill -- same values as the console printout above) --

import csv

csv_path = os.path.join(_HERE, "sim_v14_summary.csv")
with open(csv_path, "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow([
        "topic", "cosim_t0", "cosim_t1000", "t_conv",
