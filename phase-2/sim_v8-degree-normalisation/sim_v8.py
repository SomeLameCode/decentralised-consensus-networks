"""
sim_v8.py — Decentralised Consensus Learning Network (Phase 2, Run 8)

Degree-normalised peer-consistency test.

Run 7 showed that the trust penalty (mu) has zero effect on hub oligarchy
because the compounding feedback operates through COMPETENCE, not trust.
High-degree nodes accumulate more peer-consistency signal per step (more
neighbours → more terms in the sum) before any competence advantage even
begins. This structural advantage drives the oligarchy.

Fix: divide each node's peer-consistency delta_i by its degree so that
a hub with degree 7 and a leaf with degree 3 receive equally-scaled
signals regardless of neighbour count. One change to peer_consistency().

Base: sim_v7.py / sim_v5.py — N=120, SBM(4x30, p_in=0.10, p_out=0.01),
M=3, T=500, alpha=0.3, eta=0.15, lambda=0.1, epsilon=0.15,
noise=Uniform(-0.3, 0.3), seed=42, mu=0.0.

Two runs in sequence:
  Run A (baseline)  — peer_consistency unmodified (reproduces Run 7 mu=0)
  Run B (fix)       — delta_i divided by degree_i before minmax_unit()

All other code is identical. Side-by-side comparison printed at end.
"""
...
RUN_LABELS = ["baseline (no norm)", "degree-normalised"]
...
# Precompute degree normalisation: 1/degree, 0 for isolated nodes
degree_norm = np.where(degrees > 0, 1.0 / degrees, 0.0)

# -- Initial state (shared across both runs) ----------------------------------
...
for k in range(M):
    ...
    within_v8 = (diffs <= EPSILON).sum() - N
    total_pairs = N * (N - 1)
    print(f"  {TOPIC_LABELS[k]}: pairs within eps=0.3: "
          f"{within_v1}/{total_pairs} "
          f"({100 * within_v1 / total_pairs:.1f}%) | "
          f"within eps=0.15: "
          f"{within_v8}/{total_pairs} "
          f"({100 * within_v8 / total_pairs:.1f}%)")

# -- Helper functions ---------------------------------------------------------

def compute_trust(A, c_k):
    """
    Competence-weighted, row-normalised trust matrix.
    Falls back to uniform-over-adjacency for isolated nodes.
    Identical to sim_v5.py (mu=0 path from sim_v7.py).
    """
    W = A * c_k[np.newaxis, :]
...
def apply_bounded_confidence(W, b_k, epsilon):
    """
    Zero out trust to neighbours outside epsilon (D-001). Re-normalise rows.
    """
...
def peer_consistency(W, b_k, normalise_by_degree=False):
    """
    Compute delta_i = -sum_j w_ij (b_i - b_j)^2.

    If normalise_by_degree=True, divide delta_i by degree_i (precomputed as
    degree_norm). This equalises the signal magnitude across nodes regardless
    of their number of neighbours, removing the structural advantage of hubs.
    Isolated nodes (degree=0) get delta=0 in both cases.
    """
    diff_sq = (b_k[:, np.newaxis] - b_k[np.newaxis, :]) ** 2
    delta = -(W * diff_sq).sum(axis=1)
    if normalise_by_degree:
        delta = delta * degree_norm   # element-wise; degree_norm=0 for isolates
    return delta
...
def run_simulation(normalise_by_degree):
    """
    Run T=500 steps from identical initial conditions.
    normalise_by_degree controls peer_consistency calculation only.
    Noise rng re-seeded and fast-forwarded to match sim_v5.py noise sequence.
    """
    B = B0.copy()
    C = C0.copy()
...
flags = [False, True]
results = {}
for norm, label in zip(flags, RUN_LABELS):
    print(f"\n{'=' * 60}")
    print(f"Running simulation: {label}  N={N}  M={M}  T={T} ...")
    results[label] = run_simulation(normalise_by_degree=norm)
    print("  Done.")
...
BL = RUN_LABELS[0]   # baseline label
FX = RUN_LABELS[1]   # fix label

print(f"\n{'=' * 60}")
print("COMPARISON TABLE  (Run 7 mu=0 baseline vs degree-normalised)")
sep  = "+" + "-" * 36 + "+" + "-" * 22 + "+" + "-" * 22 + "+"
hdr  = f"| {'Metric':<34} | {'baseline (no norm)':^20} | {'degree-normalised':^20} |"
print(sep)
print(hdr)
print(sep)
...
# Run 5 / Run 7 reference line
print(_row("Run 5/7 top holder (reference)", "97-100%", "---"))
...
if top_bl_mean >= 0.90:
    print(f"\nValidation: baseline top-holder mean = {top_bl_mean*100:.1f}% "
          f"-- consistent with Run 5/7 (97-100%). OK.")
else:
    print(f"\nWARNING: baseline top-holder mean = {top_bl_mean*100:.1f}% -- "
          f"does NOT reproduce Run 5/7. Investigate before interpreting fix results.")

# Signal whether fix worked
top_fx_mean = results[FX]["sme_fraction"].max(axis=0).mean()
if top_fx_mean < 0.60:
    print(f"Result: degree-normalisation top-holder mean = {top_fx_mean*100:.1f}% "
          f"-- hub oligarchy suppressed (below 60%).")
elif top_fx_mean < 0.90:
    print(f"Result: degree-normalisation top-holder mean = {top_fx_mean*100:.1f}% "
          f"-- oligarchy reduced but not eliminated.")
else:
    print(f"Result: degree-normalisation top-holder mean = {top_fx_mean*100:.1f}% "
          f"-- oligarchy unchanged. Degree normalisation also ineffective.")
...
fig.suptitle(
    "sim_v8.py -- Degree-Normalised Peer-Consistency\n"
...
plt.savefig(os.path.join(_HERE, "sim_v8_plots.png"),
            dpi=150, bbox_inches="tight")
print(f"\nPlots saved -> {os.path.join(_HERE, 'sim_v8_plots.png')}")

# -- CSV export (Session 52 backfill -- same values as the console printout above) --

import csv

csv_path = os.path.join(_HERE, "sim_v8_summary.csv")
